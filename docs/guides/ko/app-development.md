...
현재 TypeScript `0.17.2` 계열은 app-level public webhook ingress용 `webhook.received` hook metadata를 지원합니다. 외부 webhook이 필요하면 임의 route를 먼저 만들지 말고 [Hook extension reference](../../reference/typescript/extensions/hook.md)의 `targetId`, `endpointToken`, 비동기 전달 제약을 확인하세요.
...
호스팅 플랫폼이 `APP_STORE_URL`이나 registration 설정을 제공한다면 그 값을 유지하세요. 독립 앱은 SDK 기본값과 auto-registration을 사용할 수 있습니다. 어느 방식이든 dependency를 lockfile로 고정하고, Function Endpoint에는 `/functions` root를 등록하며, Channel client의 WAM은 공개 `@channel.io/app-sdk-wam` hook을 사용합니다.
...
튜토리얼 README는 이 순서를 완전한 TypeScript 또는 Go project에 적용합니다. 호스팅 플랫폼이 credential, endpoint, 설치, registration을 준비했다면 제공된 값을 standalone 기본값으로 덮어쓰지 마세요.
...
`systemVersion`이 없는 command action처럼 설정된 root를 그대로 호출합니다. 호스팅 플랫폼이
bare `PUT /functions`를 기본 `/functions/v1` handler로 연결할 수 있습니다. 직접 배포할 때도 정확한
...
`server/gin.NewRoute`를 mount하세요. 호스팅 플랫폼이 이 mapping을 제공하지 않으면 bare `PUT /functions`도 versioned
...
`@channel.io/app-sdk-wam-ui`는 WAM 전용 theme, header, 상태 화면, dialog, bottom sheet, 높이 동기화에 사용합니다. 일반 UI control은 `@channel.io/bezier-react/beta`에서 직접 가져오세요. Bezier React 4는 아직 prerelease이므로 튜토리얼이 사용하는 정확한 버전을 고정합니다.

...
| Implement one unversioned `PUT /functions` route.                            | SDK servers expose `PUT /functions/:version`; configure the `/functions` root in the portal.                                                                      |
...
| Use Go 1.21 and the old tutorial's custom Fx/token/cache stack.              | Use Go 1.25 and Go SDK `v0.13.14`; the current tutorial uses the SDK registry, command builder, token manager, server, signature verifier, and auto-registration. |
...
| Hard-code an AppStore URL in application code.                               | Prefer an environment-provided `APP_STORE_URL`; use the SDK default only for standalone apps and never copy a managed runtime URL.                                |
...
- the Go quickstart now points to `v0.13.14` instead of `v0.8.3`.

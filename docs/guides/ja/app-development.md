...
TypeScript `0.17.0` の主な追加点は、app-level public webhook ingress 用の `webhook.received` hook metadata です。任意の webhook route を先に作るのではなく、[Hook extension reference](../../reference/typescript/extensions/hook.md) で `targetId`、`endpointToken`、非同期 delivery の制約を確認してください。
...
Managed generation/deploy environment では、提供された `APP_STORE_URL` と registration setting を尊重してください。Standalone app は SDK default と auto-registration を使用できます。どちらも dependency を lockfile に固定し、Function Endpoint には `/functions` root を登録し、Channel client の WAM では public `@channel.io/app-sdk-wam` hook を使います。
...
Tutorial README は、この順序を完全な TypeScript または Go project に適用します。Managed builder が credential、endpoint、install、registration を準備する場合は、runtime-provided setting を standalone default で上書きしないでください。
